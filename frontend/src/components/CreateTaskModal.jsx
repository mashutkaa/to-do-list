...
const getErrorMessage = (error) =>
  ...
  'Не вдалося створити задачу. Спробуйте ще раз.';
...
export default function CreateTaskModal({
  ...
  return (
    ...
    >
      <section
        className="max-h-[95dvh] w-full overflow-y-auto rounded-t-card bg-white shadow-2xl sm:max-w-lg sm:rounded-card"
        role="dialog"
        aria-modal="true"
      ...
      >
        <header className="flex items-start justify-between border-b border-slate-100 px-5 py-4 sm:px-6">
          <div>
            ...
            <p className="mt-0.5 text-sm text-text-muted">
              Створіть нову задачу та задайте її параметри
            </p>
          </div>
          <button
            type="button"
        ...
        >
          ...
          <footer className="grid grid-cols-1 gap-2 pt-1 sm:grid-cols-2">
            ...
            >
              {isSubmitting ? 'Додавання...' : 'Додати задачу'}
            </button>
          </footer>
        </form>
      </section>
